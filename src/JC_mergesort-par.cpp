...
void mergesort_par(int* a, int* tmp, int n, int bc);
void recmerge_par(int* a, int n, int* b, int m, int* c, int bc);
void copy_par(int *tmp, int *end, int *a, int len, int bc);
...
// parallel mergesort
// The simplest way to spawn a single thread is with the OpenMP task construct which creates one thread for the statement following the line. 
// These must be in a parallel block and in order to prevent multiple of each task starting, we also use a single block.
void mergesort_par(int* a, int* tmp, int n, int bc) {
    if (n <= bc){
        sort(a, a+n);
        return;
    ...
    int mid = n / 2;
    #pragma omp task
        mergesort_par(a, tmp, mid, bc);
        mergesort_par(a + mid, tmp + mid, n - mid, bc);
    #pragma omp taskwait

    // merge left and right into tmp and copy back into a (using STL)
    // recmerge(a, mid, a+mid, n-mid, tmp, bc);
    recmerge_par(a, mid, a+mid, n-mid, tmp, bc);
    copy(tmp, tmp+n, a);
    // copy_par(tmp, tmp+n, a, n, bc);
}
...
void copy_par(int* tmp, int* end, int* a, int len, int bc){
    ...
    int mid = len / 2;
    #pragma omp task
        copy_par(tmp, tmp+mid, a, mid, bc);
        copy_par(tmp+mid, end, a+mid, len-mid, bc);
    #pragma omp taskwait
...
void recmerge_par(int* a, int n, int* b, int m, int* c, int bc){
    ...
    #pragma omp task
        recmerge_par(a, i, b, j, c, bc);
        recmerge_par(a+i, n-i, b+j, m-j, c+i+j, bc);
    #pragma omp taskwait    
}  
